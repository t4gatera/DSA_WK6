...
// Function to perform Pre-order DFS traversal and print the tree structure
void dfsPreOrder(struct Node* root, int level) {
    if (root == NULL) return;
    ...
    // Print the current node's data (Visit the node)
    printf("%d\n", root->data);

    // Recur for the first child (going down one level)
    dfsPreOrder(root->firstChild, level + 1);

    // Recur for the next sibling (on the same level)
    dfsPreOrder(root->nextSibling, level);
}
...
int main() {
    ...
    // Perform DFS Pre-order Traversal and print the tree structure
    printf("DFS Pre-order Traversal:\n");
    dfsPreOrder(root, 0);  // Start DFS at root with level 0
